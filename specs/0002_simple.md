...
6. 报价阶段实时展示各供应商最新报价；内外部供应商首次报价后均可查看竞争力并再报价一次，第二次提交后锁定。
...
- 每份报价必须来自该询价已经邀请的供应商。内外部供应商同一询价均最多两版，且版本号连续、历史不可覆盖。
- 外部供应商完成注册后继续使用原候选供应商编号，不能额外生成一家同名供应商。
- 外部供应商注册不依赖专属邀请链接或 Token；注册完成后只能查看原供应商已经被邀请参与的询价。
- 管理端的“已提交报价数”按已提交过至少一版的供应商去重统计，不能把同一供应商的两版报价计为两家。
- 报价进行中，管理端和 Agent 可以查看每家供应商的最新总价、交期、备注、版本号和竞争力；供应商端仍不能查看其他供应商报价。
- 内外部供应商首次报价后都必须获得一份基于本次提交时有效报价的 `HIGH`、`MEDIUM` 或 `LOW` 竞争力分析；重新报价后重新分析最新版本，同时保留首版历史。
...
- 当前报价版本和版本总数；供应商有第二版时始终展示最新版。
- 内外部供应商最新报价的 `HIGH`、`MEDIUM` 或 `LOW` 竞争力。
- 距离截止时间。
...
4. 填写报价总价、交期和简单备注。
5. 确认并提交首版正式报价。
6. 查看 `HIGH`、`MEDIUM` 或 `LOW` 竞争力分析，并可在截止前重新报价一次。
7. 第二版提交后锁定；报价详情保留 V1、V2 历史，不能撤回或继续提交。
...
- 内外部供应商都可先提交首版报价，查看竞争力分析后再提交一次，因此同一询价最多两版。
- 第二次提交创建新版本，首版必须保留，不能覆盖或删除。
- 采购人员和 Agent 在 RFQ 处于 `OPEN` 时即可查看每家供应商的最新价格、交期、备注、版本号和竞争力。
- 供应商只能查看自己的报价、版本历史和竞争力，不能查看其他供应商及其报价。
- 已提交供应商数量按供应商去重；任何供应商提交第二版都不会增加“已提交家数”。
...
6. 打开 `SR-DEMO-0002`，进入内部供应商 Demo 提交首版报价，查看竞争力并提交第二版。
7. 直接打开外部供应商统一注册页，为 `EXT-SUP-DEMO-004` 提交简化资料，注册后进入 `RFQ-DEMO-0002` 并提交首版报价。
8. 查看外部供应商报价详情中的竞争力分析，并同样演示提交第二版报价。
...
3. 内部供应商首版报价后显示 `HIGH`、`MEDIUM` 或 `LOW` 竞争力，并且只允许再提交一版；两版历史均保留，管理端始终显示最新版且已提交数量仍只增加一次。
4. `EXT-SUP-DEMO-004` 注册后仍然是原外部候选供应商，并按原供应商编号看到初始化时已经存在的 `RFQ-DEMO-0002` 邀请；注册动作本身不创建邀请。
5. 外部供应商首版报价后的竞争力、一次重报、两版历史和管理端最新版规则与内部供应商一致。
