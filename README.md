...
| 内部供应商端 | `apps/internal-supplier` | <http://127.0.0.1:3001> | 选择内部供应商身份、查看询价、竞争力分析及一次重新报价 |
| 外部供应商端 | `apps/external-supplier` | <http://127.0.0.1:3002/register> | 注册外部供应商、查看询价、竞争力分析及一次重新报价 |
...
3. 在内部供应商端选择受邀身份后提交报价，查看竞争力并演示一次重新报价；
4. 在外部供应商端完成一次注册并同样演示首次报价、竞争力和一次重新报价；
...
E2E 会真实调用 DeepSeek，并覆盖创建需求、Agent 寻源、内外部供应商竞争力与一次重报、停止报价、Agent 评估、单一中选 PR 和统一重置。运行时会产生少量 DeepSeek API 调用，并会在结束时恢复固定演示数据。
...
旧迁移中仍保留报价加密相关变量，只用于兼容历史表结构；当前产品使用明文报价，内外部供应商首次报价后均可重新报价一次，不采用密封报价。
